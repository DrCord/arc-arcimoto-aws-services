...
class TestPolicyGet(unittest.TestCase):
    ...
    # test successes
    def test_policy_get_success_policy_does_exist(self):
        ...
        try:
            policy_exists = self.LambdaServiceObject.policy_get(
                lambda_arn=self.lambda_arn,
                api_arn=self.api_arn,
                mute=True
            )
    ...
    def test_policy_get_success_policy_does_not_exist(self):
        self.LambdaServiceObject.create(
            function_name=self.function_name,
            description=self.default_description,
            zip_bytes=self.LambdaServiceObject.zip_bytes,
    ...
    # test errors
    def test_policy_get_error_input_lambda_arn_invalid_type(self):
        with self.assertRaises(ValueError):
            self.LambdaServiceObject.policy_get(
                api_arn=self.api_arn,
                lambda_arn={'lambda_arn': 'dict-is-not-a-string'},
                mute=True
            )

    def test_policy_get_error_input_mute_invalid_type(self):
        with self.assertRaises(ValueError):
            self.LambdaServiceObject.policy_get(
                api_arn=self.api_arn,
                lambda_arn=self.lambda_arn,
