...
class TestLoadDependencies(unittest.TestCase, TestBundle):
    ...
    def test_load_dependencies_success(self):
        ...
        try:
            bundle.load_dependencies()
        except Exception as e:
            self.fail(f'load_dependencies failed: {e}')
        finally:
            self.DependenciesFileObject.dependencies_remove_file()

    # test errors
    def test_load_dependencies_error_file_not_found(self):
        with self.assertRaises(FileNotFoundError):
            bundle.load_dependencies()

    def test_load_dependencies_error_file_contains_invalid_json(self):
